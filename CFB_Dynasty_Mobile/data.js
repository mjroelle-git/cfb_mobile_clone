/* ============================================================
   Dynasty HQ — data & generators
   Fictional, fully customizable. No real schools/players.
   ============================================================ */
const DATA = (() => {
  ...
  // Recruiting pitch motivations (mirrors the game's pitch categories)
  ...
  // ---- fictional school generation ----
  const PLACES = ["North Valley","Lakeside","Granite","Coastal","Riverton","Fort Carson","Mesa","Summit","Iron City",
    "Bayfront","Redhawk","Cedar Falls","Silver Creek","Easthaven","Westgate","Crestline","Highland","Pinebluff",
    "Cypress","Marshall","Atlantic","Pacific","Cumberland","Sierra","Dakota","Magnolia","Liberty","Capital",
    "Calumet","Sandstone","Birchwood","Stonewall","Greenville","Frostburg","Augusta","Kingsport","Tidewater"];
  const MASCOTS = ["Wolves","Titans","Rangers","Hawks","Miners","Bison","Storm","Gators","Pioneers","Vipers",
    "Stallions","Crusaders","Thunder","Lumberjacks","Sentinels","Dukes","Royals","Mariners","Outlaws","Comets",
    "Grizzlies","Mustangs","Falcons","Privateers","Ironmen","Cyclones","Aviators","Wolverines","Cardinals","Raiders"];
  const CONFERENCES = ["Pioneer","Liberty","Coastal","Summit","Frontier","Heartland","Continental","Atlantic"];

  function genSchoolName(used){
    let name, guard = 0;
    do { name = pick(PLACES) + " " + pick(MASCOTS); guard++; }
    while (used && used.has(name) && guard < 50);
    if (used) used.add(name);
    return name;
  }

  // ---- Playbooks & scheme ----
  // weights bend the ideal depth chart toward a scheme; formations are flavor.
  const OFF_PLAYBOOKS = [
  ...
  // Ideal roster counts (sum ~85). Bent by scheme weights, then renormalized to 85.
  ...
  function schemeTargets(offWeights, defWeights){
    ...
    // force the total to exactly 85 (adjust largest groups for rounding drift)
  ...
  // overall range by star tier
  function ovrForStars(stars){
    const base = { 5:[83,90], 4:[77,85], 3:[70,80], 2:[63,73], 1:[58,68] }[stars];
    return randint(base[0], base[1]);
  }
  function potForStars(stars, ovr, gem){
    let bonus = { 5:[6,12], 4:[5,12], 3:[5,14], 2:[6,16], 1:[6,16] }[stars];
    let pot = ovr + randint(bonus[0], bonus[1]);
    if (gem) pot += randint(4, 9);
  ...
  function rollDev(stars, gem){
    let r = Math.random() + (stars-3)*0.08 + (gem?0.15:0);
    return r>0.95 ? "Elite" : r>0.78 ? "Star" : r>0.50 ? "Impact" : "Normal";
  }
  ...
  function genProspect(rivalPool, opts={}){
    ...
    if (juco){ ovr = clamp(ovr+2, 55, 95); pot = clamp(ovr + randint(2,7) + (gem?randint(2,5):0), ovr+1, 99); } // more ready, less ceiling
    ...
    const suitors = shuffle(rivalPool).slice(0, nSuitors).map(s => ({
      name: s.name, prestige: s.prestige,
      points: randint(0, 4) + s.prestige * randint(0, 2),
      isPlayer: false
    }));

    return {
      id: "p" + (_pid++),
      name, pos: posObj.p, arch: pick(posObj.arch), stars, ovr, pot,
      gem, gemKnown: false, scouted: false,
      juco, standing,
      state, height, weight: randint(w[0], w[1]),
      motivations, suitors,
      onBoard: false, priority: false, activity: null,   // activity = id of recruiting activity (persists)
      status: "open", committedTo: null, interestLog: []
    };
  }
  ...
  function randGrades(prestige){
    ...
    MOTIVATIONS.forEach(m => {
      const r = Math.random() + (prestige - 3) * 0.08;
  ...
  return {
    rand, randint, pick, chance, shuffle, clamp,
    POSITIONS, posWeights, MOTIVATIONS, STATES, CONFERENCES,
    OFF_PLAYBOOKS, DEF_PLAYBOOKS, ROSTER_BASE, schemeTargets,
    genSchoolName, genProspect, genClass, randGrades, ovrForStars, rollStars, rollDev,
    genRosterPlayer, genStartingRoster,
    FIRST, LAST, MASCOTS, PLACES
  };
})();
